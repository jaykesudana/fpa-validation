...
export async function GET(req: Request, { params }: { params: { deptId: string } }) {
  try {
    ...
    const [uploadLineRows, evidenceRows, validationLineRows] = await Promise.all([
      upload
        ? (sql`
      ...
      validationIds.length
        ? (sql`
            select r.validation_id, r.row_no, r.initiative_id, vi.name as initiative_name, r.dept_no, r.name, r.category, r.ee_id, r.country, r.frequency, r.target_date, r.identified_cents, r.notes, r.status, r.validated_cents, r.validated_date, r.status_update
    ...
    return NextResponse.json({
      ...
      validations: validationRows.map((v) => ({
        ...
        note: v.note,
        rows: validationLineRows.filter((r) => r.validation_id === v.id).map((r) => ({ ...shapeLineRow(r), status: r.status, validatedCents: Number(r.validated_cents), validatedDate: r.validated_date ?? '', statusUpdate: r.status_update ?? '' })),
      })),
